...
// Generates the next AF-#### tag (scoped per organization) and retries on a
// rare race-condition clash.
async function createAssetWithTag(organizationId, data) {
  for (let attempt = 0; attempt < 5; attempt++) {
    const count = await prisma.asset.count({ where: { organizationId } });
    const tag = `AF-${String(count + 1 + attempt).padStart(4, "0")}`;
    try {
      return await prisma.asset.create({ data: { ...data, organizationId, tag }, include: LIST_INCLUDE });
    } catch (e) {
...
export async function POST(req) {
  ...
  const asset = await createAssetWithTag(user.organizationId, { ...data, status: "AVAILABLE" });
