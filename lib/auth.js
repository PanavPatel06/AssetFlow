...
export const { handlers, auth, signIn, signOut } = NextAuth({
  ...
  callbacks: {
    // Runs on sign-in and on every subsequent request; embed id/role into the
    // token once, at sign-in (when `user` is present).
    jwt({ token, user }) {
      if (user) {
        token.id = user.id;
    ...
    // Expose id/role on the session object returned by auth().
